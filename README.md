...
| ReactIntentProvider | Intent | - `react.channel`: the identifier of the react channel that sent the message<br/> - `react.username`: the name of the react user that sent the message<br/> - `react.rawMessage`: the raw message sent by the user (before NLP processing)<br/>- `react.hostname`: the hostname of the page where the bot is accessed<br/>- `react.url`: the url of the page where the bot is accessed<br/>- `react.origin`: the origin of the page where the bot is accessed | Receive messages from the react component and translates them into Xatkit-compatible intents. Note that `react.channel`, `react.username`, and `react.rawMessage` contain the same values as `chat.channel`, `chat.username`, and `chat.rawMessage` |
